...
using Gandalan.IDAS.UniDEx.Provider.Produktion;

namespace UniDEx_Demo.Produktion
{
    public class ProduktionRoutinen
    {
        /// <summary>
        ...
        public static List<ProduktionItem> GetItems(ProduktionsInfoContainer container)
        {
            if (container == null)
                throw new ArgumentNullException("container");

            List<ProduktionItem> result = new List<ProduktionItem>();

            foreach (var groupItem in container.ContainerValues.GroupBy(k => k.GroupGuid))
            {
                // Neues Objekt erzeugen, das mit Daten gefüllt wird
                ProduktionItem businessObject = new ProduktionItem() { Kennzeichen = groupItem.Key.ToString() };
